...
export default function Calendar() {
  ...
  const [isAdmin] = useState(true); // Set to true for now since we need admin functionality
    ...
    if (storedEvents) {
      try {
        const parsedEvents = JSON.parse(storedEvents).map((event: Event) => ({
          ...event,
          // Ensure dates are properly converted to Date objects for FullCalendar
  ...
  const handleDateSelect = async (selectInfo: DateSelectArg) => {
    ...
    // Create the event with the correct date format
    ...
    try {
      // Store stringified version for localStorage
      ...
      const response = await fetch('/api/updateCalendar', {
        ...
        body: JSON.stringify({ events: eventsForStorage }),
      });
      ...
      setEvents(events); // Revert state if save failed
