...
export default function Calendar() {
  ...
  const eventContent = (eventInfo: EventContentArg) => {
    ...
    return (
      <div className="group relative" ref={eventRef}>
        ...
        <div className="hidden group-hover:block fixed z-[9999] bg-white p-4 rounded-lg shadow-xl border border-gray-200 min-w-[300px] max-w-[400px]" 
             style={{
               left: eventRef.current ? `${eventRef.current.getBoundingClientRect().left}px` : '0px',
               top: eventRef.current ? `${eventRef.current.getBoundingClientRect().bottom + window.scrollY + 5}px` : '0px'
             }}>
          {/* Arrow pointer */}
