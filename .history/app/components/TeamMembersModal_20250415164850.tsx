...
export default function TeamMembersModal({
  ...
  const handleCropComplete = async () => {
    ...
    try {
      ...
      // Get the scale of the image shown vs original size
      const scaleX = image.naturalWidth / image.width;
      const scaleY = image.naturalHeight / image.height;
      
      // Set canvas dimensions - maintain higher resolution for quality
      const cropWidthPx = crop.width * scaleX;
      const cropHeightPx = crop.height * scaleY;
      
      // Set canvas to the cropped area size - make it a reasonable size while preserving aspect ratio
      canvas.width = 500;
      canvas.height = 500;
      ...
      // Calculate the position of the crop area in the original image
      const sourceX = crop.x * scaleX;
      const sourceY = crop.y * scaleY;
      const sourceWidth = cropWidthPx;
      const sourceHeight = cropHeightPx;
      
      // Draw the exact cropped region to the canvas, maintaining aspect ratio
      ctx.drawImage(
        image,
        sourceX,
        sourceY,
      ...
      canvas.toBlob(async (blob) => {
        ...
        // Update preview by creating a URL for the blob
        const croppedImageUrl = URL.createObjectURL(blob);
        
        // Immediately update preview in UI
                  ...
                  <div className="flex flex-col items-center space-y-2">
                    <div className="flex space-x-2 w-full">
                      ...
                      {imagePreview[member.id] && (
                        <button
                          onClick={() => {
                            setTempImageUrl(imagePreview[member.id]);
